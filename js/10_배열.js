...
function orderFn() {
    ...
    const menus = ["라면", "김밥", "샌드위치", "우동"]; // 메뉴 배열
    const prices = [4000, 3000, 5000, 6000]; // 가격 배열

    // 주문한 메뉴 카운트 배열
    const counts = new Array(menus.length); // 메뉴 개수 만큼의 크기를 가지는 배열
    ...
    while (true) { // 무한 반복
        ...
        if (input.length == 0 || isNaN(Number(input))) { // 수량을 입력하지 않았거나, 숫자를 입력하지 않은 경우
            alert("잘 못 입력 하셨습니다 다시 주문해주세요.");
            continue;
        }
    ...
    tbody.innerHTML = ""; // tbody 이전 내용 지우기


    let sum = 0; // 합계를 구하기 위한 변수
    ...
    total.innerText = sum + "\t원"; // 합계 화면에 출력
}
